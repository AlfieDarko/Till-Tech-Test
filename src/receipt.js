  ...
  class Receipt {
    ...
    printReceipt(args) {
      ...
      function discountDisplayer(args) {
        if (self.discount.returnMuffinDiscountBool(args) &&
          self.discount.returnSpendOver50DiscountBool(totalCalculated)) {
          receiptArray.push("10% Muffin Discount!", "5% Off Purchases over 50 Discount!");
        }
      }
      // Is this function finished properly? 
      ...
      let itemsTotalDiscounts = discount.applyDiscounts(total.calculate(args), args);
      let itemsTotalWithoutTax = total.calculate(args).toFixed(2);
      console.log(itemsTotalWithoutTax, "withouttax");
      let amountToTax = (tax.applyTax(total.calculate(args)).toFixed(2) -
        total.calculate(args).toFixed(2)).toFixed(2);
      let totalWithTax = parseFloat(itemsTotalDiscounts) + parseFloat(amountToTax);
      // Now we add all the info to an receipts array
      // discountDisplayer(args);
      if (discount.returnMuffinDiscountBool(args) &&
