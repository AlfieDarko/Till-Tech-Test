(function(exports) {
  class Receipt {
    ...
    addDiscountAlertOnReceipt(args, totalCalculated) {
      if (
        this.discount.returnMuffinDiscountBool(args) &&
        this.discount.returnSpendOver50DiscountBool(totalCalculated)
      ) {
        _receiptArray.push(
          "10% Muffin Discount!",
          "5% Off Purchases over 50 Discount!"
        );
      } else if (this.discount.returnMuffinDiscountBool(args)) {
    ...
    createReceipt(basket) {
      ...
      let preTotal = this.total.calculate(basket).toFixed(2);
      let preTotalWithDiscounts = this.discount.applyDiscounts(preTotal, basket);
      _receiptArray.push(`${basket.name}'s Order:`);
      let amountToTax = (
        this.tax.applyTax(total.calculate(basket)).toFixed(2) - preTotal
      ).toFixed(2);
      let totalWithTax =
      ...
      this.zipItemsAndPrices(basket.items, this.total.calculateLinePrice(basket)).map(
        lineItem => {
